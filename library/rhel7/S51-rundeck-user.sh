#!/bin/bash
#
# Adding/Configuring the 'rundeck' user.

. /etc/sdi/thig-settings

#Logic to narrow shot grouping
if [[ -z "$ENVIRONMENT" ]]; then 
	echo "No environment variable defined" && exit 0;
	
elif [[ ! -d "/opt/sdi/thig-application-configs-global/roles/flgnv/RHEL7/rdk/all/certs/" ]]; then
	echo "Needed pubkey directory doesn't exist" && exit 0;
	
elif [[ `cat /etc/passwd | grep "rundeck"` ]]; then
	echo "Already a user Rundeck on this server" && exit 0;
